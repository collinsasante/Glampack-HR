...
describe("Attendance check-in/check-out", () => {
  it("checks in with GPS position parsed into discrete columns, distance-from-office computed server-side", async () => {
    ...
    const res = await request(app)
      ...
      .send({ shift: "MorningProductionDay", position: { lat: 5.6037, lng: -0.187 }, city: "Accra", region: "Greater Accra" });

    expect(res.status).toBe(201);
    expect(res.body.checkInMethod).toBe("GPS");
    expect(Number(res.body.checkInLat)).toBeCloseTo(5.6037, 4);
    // Real office is at 5.603717,-0.186964 (attendance-tracker.html) — this position is
    // ~100m away, so distance should be small but computed, never a client-submitted value.

...
export function MedicalClaimsPagination({
  page,
  pageSize,
  total,
  onPageChange,
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 px-1 py-1">
      <p className="text-sm text-muted-foreground">
        Showing {from}–{to} of {total} claim{total === 1 ? "" : "s"}
      </p>
        ...
        <div className="flex items-center gap-1">
          ...
          {pageNumbers.map((n, i) =>
            n === "ellipsis" ? (
              ...
              <Button key={n} variant={n === page ? "default" : "outline"} size="icon-sm" onClick={() => onPageChange(n)}>
                {n + 1}
              </Button>
            )
          )}
